...
import { formatDate, formatPrice, formatPriceVND } from '@/lib/formatters';
...
export default async function PurchasePage({
    ...
    return (
        <div className="container my-6 px-12">
            <PageHeader title="Purchase Hisotry" />
...
async function SuspenseBoundary({ purchaseId }: { purchaseId: string }) {
    ...
    let pricingRows: { label: string; amountInDollars: number; isBold?: boolean }[] = [];

    const coupon = await getUserCoupon();

    if (purchase.paymentProvider === 'stripe') {
        ({ receiptUrl, pricingRows } = await getStripeDetails(
            purchase.stripeSessionId!,
            purchase.pricePaidInCents,
            purchase.refundedAt != null,
        ));
    } else if (purchase.paymentProvider === 'vnpay') {
        ({ receiptUrl, pricingRows } = getVnpayDetails(
            purchase.vnpTransactionId!,
            purchase.pricePaidInCents,
            coupon?.discountPercentage ?? 0,
            purchase.refundedAt != null,
        ));
    }
    //  else if (purchase.paymentProvider === 'momo') {
    //     ({ receiptUrl, pricingRows } = getMomoDetails(
    //         purchase.momoTransactionId!,
    //         purchase.pricePaidInCents,
    //     ));
    // }
    ...
    return (
        <>
            <PageHeader title={purchase.productDetails.name}>
                {receiptUrl && purchase.paymentProvider === 'stripe' && (
            ...
            <Card>
                <CardHeader className="pb-4">
                    <div className="flex justify-between items-start gap-4">
                        ...
                        <Badge className="test-base">
                            {purchase.refundedAt ? 'Refunded' : 'Paid'}
                        </Badge>
                    </div>
                </CardHeader>
                <CardContent className="pb-4 grid grid-cols-3 gap-8 border-t pt-4">
                    <div>
                    ...
                    <div>
                        ...
                        <div>Huy is course</div>
                    </div>
                    <div>
                        <label className="text-sm text-muted-foreground">Payment Method</label>
                        <div className="capitalize">{purchase.paymentProvider}</div>
                    </div>
                </CardContent>
                        ...
                        <Fragment key={label}>
                            ...
                            <div className={cn('justify-self-end', isBold && 'font-bold')}>
                                {purchase.paymentProvider === 'stripe'
                                    ...
                                    : formatPriceVND(amountInDollars, { showZeroAsNumber: true })}
                            </div>
                        </Fragment>
                    ))}
                </CardFooter>
...
async function getStripeDetails(
    ...
    const { payment_intent, total_details, amount_total, amount_subtotal } =
        await stripeServerClient.checkout.sessions.retrieve(stripeSessionId, {
            expand: ['payment_intent.latest_charge', 'total_details.breakdown.discounts'],
        });

    const refundAmount =
        typeof payment_intent !== 'string' && typeof payment_intent?.latest_charge !== 'string'
...
function getVnpayDetails(
    vnpTransactionId: string,
    pricePaidInCents: number,
    discountPercent: number,
    isRefunded: boolean,
) {
    const subtotal = (pricePaidInCents * 100) / discountPercent;
    const discount = subtotal * discountPercent;
    const total = subtotal - discount;
    const refundAmount = isRefunded ? pricePaidInCents : undefined;

    return {
        receiptUrl: `/vnpay/receipt/${vnpTransactionId}`,
        pricingRows: getPricingRows(null, {
            subtotal,
            total,
            refund: refundAmount,
            discountRows: [
...
function getPricingRows(
    totalDetails: Stripe.Checkout.Session.TotalDetails | null,
    {
        ...
        subtotal: number;
        refund?: number;
        discountRows?: { label: string; amount: number }[];
    },
) {
